...
$\mathbf{X}_{ij}=(1,X_{ij1},...,X_{ijs})^\top$ is a $1 \times (s+1)$ vector and  
...
The kinship matrix (kin2) is a positive semi-definite relationship matrix (e.g. kinship matrix in genetic association studies). The rownames and colnames of this block matrices must at least include all samples as specified in the ped column of the pedigree file. Below, we have the first 6 rows of the kinship matrix example  (first $6$ columns):
...
In the two functions CBGF_Beta_Only and CBGF_Fixed, we fixed, by default, the number of B-spline basis functions as $K = K_{\beta} = 10,$ and the number of Fourier basis functions to be $K = K_{\beta} = 11.$ The order of the basis functions was fixed as $4$.
