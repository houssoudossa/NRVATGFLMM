CBGF_Beta_Only <-
function(n, gamm, heritability, a1=1, b1=25, Ped, covariates, geno, kin2, pos, order, beta_basis_BO_BS, base_BO_BS = "bspline", beta_basis_BO_FS,base_BO_FS = "fspline"){
  ...
  Beta.Smooth = function(geno, pos, order, beta_basis, base){
    ### CREATE BASIS
    #genos=G; pos=map[,4]; beta_basis = 10; base="bspline"; order=4
    if (base ==  "bspline")
    {
  ...
  ##############################################
  ######### Beta_Only b.spline function ##################
  
  UJ_BO_BS= Beta.Smooth(geno,pos, order, beta_basis_BO_BS, base_BO_BS)
  
  ############ Beta_Only Fourier ################## 
  #Getting Weight for the SNP 
  p_BO_BS=apply(UJ_BO_BS, 2, function(x) {sum(x, na.rm = T)/((length(x) - sum(is.na(x))) * 2)})
  
  W_BO_BS=Generate.W(a1,b1,p_BO_BS)
  ...
  AL_BO_BS=KNL(Z=UJ_BO_BS, kernel="linear",weights=W_BO_BS, n=sum(n),m=ncol(UJ_BO_BS),rho=ncol(UJ_BO_BS),gamma=1,d=2)
  KL_BO_BS=sqrt.sigmaY%*%AL_BO_BS%*%sqrt.sigmaY 
  eig_BO_BS=eigen(KL_BO_BS)
  DL_BO_BS=eig_BO_BS$values
  
  ### Computation of the P.Values
  S_obsL_BO_BS=t(Y-mu.hat) %*% AL_BO_BS %*%(Y-mu.hat)
  p.valueL_BO_BS=davies(S_obsL_BO_BS, DL_BO_BS)$Qq
  
  
  ### Computation of the Quadratic Kernel Functions
  AQ_BO_BS=KNL(Z=UJ_BO_BS, kernel="quadratic",weights=W_BO_BS, n=sum(n),m=ncol(UJ_BO_BS),rho=ncol(UJ_BO_BS),gamma=1,d=2)
  KQ_BO_BS=sqrt.sigmaY%*%AQ_BO_BS%*%sqrt.sigmaY 
  eig_BO_BS=eigen(KQ_BO_BS)
  DQ_BO_BS=eig_BO_BS$values
  
  ### Computation of the P.Values
  S_obsQ_BO_BS=t(Y-mu.hat) %*% AQ_BO_BS %*%(Y-mu.hat)
  p.valueQ_BO_BS=davies(S_obsQ_BO_BS, DQ_BO_BS)$Qq
  
  
  ### Computation of the IBS Kernel Functions
  #AIB_BO_BS=KNL(Z=UJ_BO_BS, kernel="IBS",weights=W_BO_BS, n=sum(n),m=ncol(UJ_BO_BS),rho=ncol(UJ_BO_BS),gamma=1,d=2)
  #KIB_BO_BS=sqrt.sigmaY%*%AIB_BO_BS%*%sqrt.sigmaY 
  #eig_BO_BS=eigen(KIB_BO_BS)
  #DIB_BO_BS=eig_BO_BS$values
  #DQ[which(DQ<0)]=0
  ### Computation of the P.Values
  #S_obsIB_BO_BS=t(Y-mu.hat) %*% AIB_BO_BS %*%(Y-mu.hat)
  ###Get the Quadratic Statistic and P.value
  #p.valueIB_BO_BS=davies(S_obsIB_BO_BS, DIB_BO_BS)$Qq
  
  
  ### Computation of the Gaussien Kernel Functions
  AG_BO_BS=KNL(Z=UJ_BO_BS, kernel="gauss",weights=W_BO_BS, n=sum(n),m=ncol(UJ_BO_BS),rho=ncol(UJ_BO_BS),gamma=1,d=2)
  KG_BO_BS=sqrt.sigmaY%*%AG_BO_BS%*%sqrt.sigmaY 
  eig_BO_BS=eigen(KG_BO_BS)
  DG_BO_BS=eig_BO_BS$values
  #DG[which(DG<0)]=0
  ### Computation of the P.Values
  S_obsG_BO_BS=t(Y-mu.hat) %*% AG_BO_BS %*%(Y-mu.hat)
  ###Get the Gaussien Statistic and P.values
  p.valueG_BO_BS=davies(S_obsG_BO_BS, DG_BO_BS)$Qq
  
  
  ### Computation of the Polynomial Kernel Functions
  AP_BO_BS=KNL(Z=UJ_BO_BS, kernel="poly",weights=W_BO_BS, n=sum(n),m=ncol(UJ_BO_BS),rho=ncol(UJ_BO_BS),gamma=1,d=2)
  KP_BO_BS=sqrt.sigmaY%*%AP_BO_BS%*%sqrt.sigmaY 
  eig_BO_BS=eigen(KP_BO_BS)
  DP_BO_BS=eig_BO_BS$values
  #DG[which(DG<0)]=0
  ### Computation of the P.Values
  S_obsP_BO_BS=t(Y-mu.hat) %*% AP_BO_BS %*%(Y-mu.hat)
  ###Get the Gaussien Statistic and P.values
  p.valueP_BO_BS=davies(S_obsP_BO_BS, DP_BO_BS)$Qq
  
  
  ##############################################
  ######### Beta_Only f.spline function ##################
  UJ_BO_FS= Beta.Smooth(geno,pos, order, beta_basis_BO_FS, base_BO_FS)
  
  ############ Beta_Only Fourier ################## 
  #Getting Weight for the SNP 
  p_BO_FS=apply(UJ_BO_FS, 2, function(x) {sum(x, na.rm = T)/((length(x) - sum(is.na(x))) * 2)})
  
  W_BO_FS=Generate.W(a1,b1,p_BO_FS)
  
  ############## NRVATGFLMM ###############################
  ### Computation of the Linear Kernel Functions
  AL_BO_FS=KNL(Z=UJ_BO_FS, kernel="linear",weights=W_BO_FS, n=sum(n),m=ncol(UJ_BO_FS),rho=ncol(UJ_BO_FS),gamma=1,d=2)
  KL_BO_FS=sqrt.sigmaY%*%AL_BO_FS%*%sqrt.sigmaY 
  eig_BO_FS=eigen(KL_BO_FS)
  DL_BO_FS=eig_BO_FS$values
  
  ### Computation of the P.Values
  S_obsL_BO_FS=t(Y-mu.hat) %*% AL_BO_FS %*%(Y-mu.hat)
  p.valueL_BO_FS=davies(S_obsL_BO_FS, DL_BO_FS)$Qq
  
  
  ### Computation of the Quadratic Kernel Functions
  AQ_BO_FS=KNL(Z=UJ_BO_FS, kernel="quadratic",weights=W_BO_FS, n=sum(n),m=ncol(UJ_BO_FS),rho=ncol(UJ_BO_FS),gamma=1,d=2)
  KQ_BO_FS=sqrt.sigmaY%*%AQ_BO_FS%*%sqrt.sigmaY 
  eig_BO_FS=eigen(KQ_BO_FS)
  DQ_BO_FS=eig_BO_FS$values
  
  ### Computation of the P.Values
  S_obsQ_BO_FS=t(Y-mu.hat) %*% AQ_BO_FS %*%(Y-mu.hat)
  p.valueQ_BO_FS=davies(S_obsQ_BO_FS, DQ_BO_FS)$Qq
  
  
  ### Computation of the IBS Kernel Functions
  #AIB_BO_FS=KNL(Z=UJ_BO_FS, kernel="IBS",weights=W_BO_FS, n=sum(n),m=ncol(UJ_BO_FS),rho=ncol(UJ_BO_FS),gamma=1,d=2)
  #KIB_BO_FS=sqrt.sigmaY%*%AIB_BO_FS%*%sqrt.sigmaY 
  #eig_BO_FS=eigen(KIB_BO_FS)
  #DIB_BO_FS=eig_BO_FS$values
  #DQ[which(DQ<0)]=0
  ### Computation of the P.Values
  #S_obsIB_BO_FS=t(Y-mu.hat) %*% AIB_BO_FS %*%(Y-mu.hat)
  ###Get the Quadratic Statistic and P.value
  #p.valueIB_BO_FS=davies(S_obsIB_BO_FS, DIB_BO_FS)$Qq
  
  
  ### Computation of the Gaussien Kernel Functions
  AG_BO_FS=KNL(Z=UJ_BO_FS, kernel="gauss",weights=W_BO_FS, n=sum(n),m=ncol(UJ_BO_FS),rho=ncol(UJ_BO_FS),gamma=1,d=2)
  KG_BO_FS=sqrt.sigmaY%*%AG_BO_FS%*%sqrt.sigmaY 
  eig_BO_FS=eigen(KG_BO_FS)
  DG_BO_FS=eig_BO_FS$values
  #DG[which(DG<0)]=0
  ### Computation of the P.Values
  S_obsG_BO_FS=t(Y-mu.hat) %*% AG_BO_FS %*%(Y-mu.hat)
  ###Get the Gaussien Statistic and P.values
  p.valueG_BO_FS=davies(S_obsG_BO_FS, DG_BO_FS)$Qq
  
  
  ### Computation of the Polynomial Kernel Functions
  AP_BO_FS=KNL(Z=UJ_BO_FS, kernel="poly",weights=W_BO_FS, n=sum(n),m=ncol(UJ_BO_FS),rho=ncol(UJ_BO_FS),gamma=1,d=2)
  KP_BO_FS=sqrt.sigmaY%*%AP_BO_FS%*%sqrt.sigmaY 
  eig_BO_FS=eigen(KP_BO_FS)
  DP_BO_FS=eig_BO_FS$values
  #DG[which(DG<0)]=0
  ### Computation of the P.Values
  S_obsP_BO_FS=t(Y-mu.hat) %*% AP_BO_FS %*%(Y-mu.hat)
  ###Get the Gaussien Statistic and P.values
  p.valueP_BO_FS=davies(S_obsP_BO_FS, DP_BO_FS)$Qq
  
  ####### The Results of the Statistcs and P.values
  Beta_Only=cbind(gamm.hat[1], gamm.hat[2],gamm.hat[3], heritability_est, S_obsL_BO_BS, p.valueL_BO_BS,S_obsQ_BO_BS,p.valueQ_BO_BS, S_obsG_BO_BS, p.valueG_BO_BS, S_obsP_BO_BS, p.valueP_BO_BS, S_obsL_BO_FS, p.valueL_BO_FS, S_obsQ_BO_FS, p.valueQ_BO_FS, S_obsG_BO_FS, p.valueG_BO_FS, S_obsP_BO_FS, p.valueP_BO_FS)
  colnames(Beta_Only) =c("Intercept", "gam1", "gam2", "h.S_estim", "StatL.Beta_Bs", "p.v_obsbL.Beta_Bs", "StatQ.Beta_Bs", "p.v_obsbQ.Beta_Bs", "StatG.Beta_Bs", "p.v_obsbG.Beta_Bs", "StatP.Beta_Bs", "p.v_obsbP.Beta_Bs", "StatL.Beta_Fs", "p.v_obsbL.Beta_Fs","StatQ.Beta_Fs", "p.v_obsbQ.Beta_Fs", "StatG.Beta_Fs", "p.v_obsbG.Beta_Fs", "StatP.Beta_Fs", "p.v_obsbP.Beta_Fs") 
  rownames(Beta_Only) = NULL
  return(as.data.frame(Beta_Only))
}
